...
class SpikingDeepGalerkinNet(nn.Module):
    def __init__(self, num_hidden_layers=4, num_hidden_units=50, T_sim=10, v_threshold=1.0, v_reset=0.0, device='cpu', d=1):
        """
        ...
        self.lif_layers.append(snn.RLeaky(
            ...
            linear_features=num_hidden_units  # Specify output features
        ))
        ...
        self.lif_layers.append(snn.Leaky(
            beta=torch.ones(1) * beta,  # Match output dimension
            threshold=torch.ones(1) * v_threshold,
    ...
    def forward(self, t, x, return_spikes=False):
        """
        ...
        for i in range(len(self.lif_layers)):
            if i < len(self.lif_layers) - 1:
                mem_states.append(self.lif_layers[i].init_rleaky())
            else:
                mem_states.append(self.lif_layers[i].init_leaky())
        ...
        for step in range(self.T_sim):
            ...
            for i, (fc, lif) in enumerate(zip(self.layers, self.lif_layers)):
                if i < len(self.layers) - 1:
                    ...
                    spk, mem = lif(x_step, mem_states[i][0],mem_states[i][1])  # Spikes and membrane potential
                    mem_states_list = list(mem_states[i])
                    mem_states_list[0] = spk  # Update membrane state
                    mem_states_list[1] = mem

                    mem_states[i] = tuple(mem_states_list)

                    x_step = spk  # Pass spikes to next layer
                    if i < len(self.layers) - 2:
                        spk_rec[i].append(float(torch.mean(spk)))
                else:
                    cur_out = fc(mem_states[i-1][1])  # Use previous layer's membrane potential
                    spk_out, mem = lif(cur_out, mem_states[i])
                    mem_output = mem  # Shape: [batch_size, 1]
        ...
        return torch.mean(mem_out_rec, dim=0)  # Shape: [batch_size, 1]
